...
void kill_process(const pid_t pid)
{
...
void judge_process(const int input, const pid_t pid)
{
	int i;
	int * temp;
	...
	if(judge_process_each_time() != ABNORMAL)
		printf("\tCongratulations!\tThis is one normal syscall sequence for now!\n");
	else{
